...
function formatEventLine(e, opponentName) {
  ...
  if (t.includes('penalty') && t.includes('missed')) {
    ...
    return sanitize(`${min}Rigore fallito ${who}`);
  }
  // Shots
  if (t === 'missed-shot' || t === 'opponent-missed-shot') {
    const isVig = t === 'missed-shot';
    return sanitize(`${min}Tiro fuori ${isVig ? (ps||'Vigontina') : opp}`);
  }
  if (t === 'shot-blocked' || t === 'opponent-shot-blocked') {
    const isVig = t === 'shot-blocked';
    return sanitize(`${min}${isVig ? (ps||'Vigontina') : opp} tiro parato`);
  }
  if (t === 'save' || t === 'opponent-save') {
    const isVig = t === 'save';
    return sanitize(`${min}Parata ${isVig ? (ps||'Vigontina') : `portiere ${opp}`}`);
  }
  // Woodwork
  if (t?.includes('palo-') || t?.includes('traversa-')) {
    const isVig = e.team === 'vigontina';
    const hit = e.hitType === 'palo' ? 'Palo' : 'Traversa';
    return sanitize(`${min}${hit} ${isVig ? (ps||'Vigontina') : opp}`);
  }
  // Free kicks
  if (t?.startsWith('free-kick')) {
    const isOpp = t.includes('opponent');
    if (t.includes('missed')) return sanitize(`${min}Punizione fuori ${isOpp ? opp : (ps||'')}`);
    if (t.includes('saved')) return sanitize(`${min}Punizione parata ${isOpp ? opp : (ps||'')}`);
    if (t.includes('hit')) return sanitize(`${min}Punizione ${e.hitType==='palo'?'Palo':'Traversa'} ${isOpp ? opp : (ps||'')}`);
  }
  // Substitution
  if (t === 'substitution') {
    const out = e.out ? sanitize(`${e.out?.num||''} ${e.out?.name||''}`) : '';
    const inn = e.in ? sanitize(`${e.in?.num||''} ${e.in?.name||''}`) : '';
    return sanitize(`${min}Sostituzione: ${out} -> ${inn}`); // use ASCII arrow to avoid font issues
  }
  // Fallback
