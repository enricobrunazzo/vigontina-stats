...
function eventLabel(e, opponentName = "Avversari") {
  const min = e.minute != null ? `${e.minute}'` : ""; const who = e.scorerName || e.scorer || "";
  const assist = e.assistName || (e.assist != null ? ` (assist: ${e.assist})` : "");
  switch (e.type) {
    case "goal": return `${min} - Gol ${who}${assist ? " " + assist : ""}`;
    case "own-goal": return `${min} - Autogol`;
    case "opponent-goal": return `${min} - Gol ${opponentName}`;
    case "penalty-goal": return `${min} - Gol (Rig.) ${who}`;
    case "penalty-missed": return `${min} - Rigore fallito`;
...
export const exportMatchToExcel = async (match) => {
  ...
  } catch (error) { console.error('Errore export Excel:', error); alert(`Errore durante l'esportazione Excel: ${error.message ?? 'Errore sconosciuto'}`); }
};
...
export const exportFIGCReportPDF = async (match, figcData) => {
  if (!match) return; const doc = new jsPDF({ unit: "pt", format: "a4" });
  const fileName = `Rapporto_FIGC_${match.opponent.replace(/\s+/g, "_")}_${fmtDateIT(match.date)}.pdf`; doc.save(fileName);
};