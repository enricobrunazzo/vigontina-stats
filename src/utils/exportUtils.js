...
// Formattazione evento migliorata per PDF/Excel: include tutti i nuovi tipi
function eventLabel(e, opponentName = "Avversari") {
  ...
  switch (e.type) {
    ...
    case "save":
      return `${min} - Parata ${e.player ? `${e.player} ${e.playerName || ''}`.trim() : ''}`.trim();
    case "opponent-save":
      return `${min} - Parata portiere ${opponentName}`;
    case "missed-shot":
      return `${min} - Tiro fuori ${e.player ? `${e.player} ${e.playerName || ''}`.trim() : ''}`.trim();
    case "opponent-missed-shot":
      return `${min} - Tiro fuori ${opponentName}`;
    case "shot-blocked":
      return `${min} - Tiro parato ${e.player ? `${e.player} ${e.playerName || ''}`.trim() : ''}`.trim();
    case "opponent-shot-blocked":
      return `${min} - Tiro parato ${opponentName}`;
    default:
      if (isHit) {
        const who = e.team === 'vigontina' ? `${e.player || ''} ${e.playerName || ''}`.trim() : opponentName;
        return `${min} - ${hitLabel || 'Legno'} ${who}`.trim();
      }
      return `${min} - Evento`;
  }
}
