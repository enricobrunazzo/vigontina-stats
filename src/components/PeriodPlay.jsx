...
const PeriodPlay = ({
  ...
  // Guards to prevent render crashes
  ...
  const confirmShotForTeam = (team) => {
    ...
    if (team === 'vigontina') {
      ...
      if (pendingShotOutcome === 'fuori') onAddMissedShot('opponent', null, safeGetMinute);
      else if (pendingShotOutcome === 'parato') onAddShotBlocked('opponent', null, safeGetMinute);
      else if (pendingShotOutcome === 'palo' || pendingShotOutcome === 'traversa') onAddPostCrossbar(pendingShotOutcome, 'opponent', null, safeGetMinute);
  ...
  const confirmShotForPlayer = (playerNum) => {
    ...
    if (pendingShotOutcome === 'fuori') onAddMissedShot('vigontina', playerNum, safeGetMinute);
    else if (pendingShotOutcome === 'parato') onAddShotBlocked('vigontina', playerNum, safeGetMinute);
    else if (pendingShotOutcome === 'palo' || pendingShotOutcome === 'traversa') onAddPostCrossbar(pendingShotOutcome, 'vigontina', playerNum, safeGetMinute);
    setPendingShotOutcome(null);
  };
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-md">
              ...
              <div className="space-y-3">
                ...
                <button onClick={() => { setShowShotTeamDialog(false); confirmShotForTeam('opponent'); }} className="w-full bg-blue-600 text-white p-3 rounded hover:bg-blue-700 font-medium">{match.opponent}</button>
